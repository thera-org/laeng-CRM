...
alter table public.diario_obras_fotos        enable row level security;
alter table public.planejamento_obras        enable row level security;
alter table public.planejamento_atividades   enable row level security;

-- Permissive policies for authenticated users (matches existing tables)
do $$
begin
  -- diario_obras
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='diario_obras' and policyname='diario_obras_all_auth') then
    create policy diario_obras_all_auth on public.diario_obras
      for all to authenticated using (true) with check (true);
  end if;

  -- diario_obras_fotos
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='diario_obras_fotos' and policyname='diario_obras_fotos_all_auth') then
    create policy diario_obras_fotos_all_auth on public.diario_obras_fotos
      for all to authenticated using (true) with check (true);
  end if;

  -- planejamento_obras
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='planejamento_obras' and policyname='planejamento_obras_all_auth') then
    create policy planejamento_obras_all_auth on public.planejamento_obras
      for all to authenticated using (true) with check (true);
  end if;

  -- planejamento_atividades
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='planejamento_atividades' and policyname='planejamento_atividades_all_auth') then
    create policy planejamento_atividades_all_auth on public.planejamento_atividades
      for all to authenticated using (true) with check (true);
  end if;
end $$;
